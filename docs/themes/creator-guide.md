...
**Quick appearance** in Appearance provides notebook-only accent, editor font family and editor font size overrides. The size slider runs from 11 to 28 px. Choose Theme font or Use theme fonts to restore the author’s typography. Font choices use bundled Inter or system serif/monospace fonts, with no downloads. Title-bar styling, including **Colored title bar**, belongs in the theme editor. Older quick title-bar overrides are ignored so the selected theme controls it. Quick overrides and manual navigation/sidebar changes do not edit the shared theme. Plasma settings live in the Theme Editor under Advanced surfaces, where authors can preview and save them together with the theme. Choosing a theme again applies its visual defaults and clears the quick appearance overrides. Navigation style and sidebar visibility stay as you set them. When they differ from the theme's defaults, **Use theme's layout** in Appearance applies the author's navigation and right sidebar choices. The preview always starts with the author's intended layout.
