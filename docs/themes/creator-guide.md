...
3. Choose the author's default navigation and right-sidebar behavior. All three navigation styles are supported. Readers apply these with **Use theme defaults → Use theme default layout options**; switching themes preserves their existing choices. **Keep current** leaves that choice alone even when applying the layout.
...
The saved theme list refreshes automatically when Appearance opens or the Tigrana window regains focus. Themes added or edited outside the app appear without a manual refresh. This updates the list without changing your active theme or an unfinished editor draft.

If you adjust Appearance after choosing a theme, **Save current settings as new theme** appears beside the theme actions. It opens a new draft with the current accent, editor font and size, navigation, right sidebar and Plasma settings. Name it and save to reuse those choices. The original theme stays intact. Returning your choices to the theme defaults removes the reminder. Light/dark mode is a notebook preference; both palettes are kept in the new theme.
...
**Quick appearance** in Appearance provides notebook-only accent, editor font family and editor font size overrides. The size slider runs from 11 to 28 px. A reset icon appears beside each value that differs from the theme default; it resets only that value, keeping your other choices. The Theme font option shows the author’s font name in parentheses. Choose it to use the author’s font family. **Use theme defaults**, beside Save current settings as new theme, offers three choices: restore all theme defaults, restore only fonts and colors, or restore only layout options. Layout includes navigation, right sidebar visibility, editor width, alignment and word count. Missing author defaults keep your current choice. The fonts-and-colors option preserves layout and Plasma settings; the all-defaults option also restores Plasma and title-bar settings. Font choices include bundled Inter, IBM Plex Mono, Solway, and VT323, plus system font, serif, and monospace fallbacks, with no downloads. Saved themes include any selected packaged font and its license notices. Title-bar styling, including **Colored title bar**, belongs in the theme editor. Older quick title-bar overrides are ignored so the selected theme controls it. Quick overrides and manual navigation/sidebar changes do not edit the shared theme. Plasma settings live in the Theme Editor under Advanced surfaces, where authors can preview and save them together with the theme. Choosing a theme again applies its visual defaults and clears the quick appearance overrides. Navigation style and sidebar visibility stay as you set them. When they differ from the theme's defaults, **Use theme defaults → Use theme default layout options** in Appearance applies the author's navigation and right sidebar choices. The preview always starts with the author's intended layout.
...
In Visual, **Default editor width** and **Default note alignment** apply when someone selects the theme. The built-in themes use Keep current for both. **Default word count** can show or hide the word count when the theme is selected; all built-in themes use Keep current. Alignment positions the note column, not the text inside paragraphs. **Keep current** leaves that choice alone. Manual changes in View or editor options are saved with the notebook; selecting a theme with explicit defaults reapplies them.
...
If a theme breaks, open Appearance and choose another theme or **Restore default appearance**. Restoring Default resets colors, fonts, effects, and layout: Comfortable Width, Align Center, Dual Pane with Sections, both sidebars, and word count. Sidebars still collapse automatically when the window is too narrow. Your light/dark/system choice stays unchanged. The recovery shortcut is **Cmd+Option+Shift+T** on macOS or **Ctrl+Alt+Shift+T** elsewhere. Invalid notebook snapshots fall back to a safe Default appearance in memory with a notice; their saved content is kept until you choose a replacement. A bad bundled entry is isolated at startup, while the build checks still reject invalid shipped themes.
