...
mod notebook_paths;
use link_index::{
    ensure_note_id_in_content, ensure_note_id_in_content_with_preferred, forget_path_from_index,
    forget_subtree_from_index, link_index_path, move_index_path, parse_links_for_note,
    read_frontmatter_field, read_link_index_file, rebuild_index_for_root, reindex_note_after_save,
    repair_inbound_links, repair_subtree_paths, set_note_id_in_content, split_frontmatter,
    unique_note_relative, update_index_links_for_source, write_folder_sidecar,
    write_link_index_file, FolderRecord, FolderSidecar, LinkIndex, NoteRecord,
};
use notebook_paths::{
...
fn note_history_reason(existing: &str, incoming: &str) -> String {
...
fn create_note_version_snapshot(
    root: &Path,
    path: &str,
...
fn list_note_versions(payload: NoteVersionsPayload) -> Result<Vec<NoteVersionEntry>, String> {
    ...
    let current_id = note_id_from_content(&current_content).or_else(|| {
        read_link_index_file(&root)
            .path_to_id
            .get(&payload.path)
            .cloned()
    });
    let key = note_history_key(&payload.path, current_id.as_deref());
    let mut index = read_note_history_index(&root).unwrap_or_default();
    cleanup_note_history(&root, &mut index);
...
fn restore_note_version(payload: NoteVersionPayload) -> Result<String, String> {
    ...
    let current_key = note_history_key(&payload.path, current_id.as_deref());
