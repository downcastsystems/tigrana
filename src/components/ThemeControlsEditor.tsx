...
export function ThemeControlsEditor({ theme, change }: { theme: ThemeDocument; change: (patch: Partial<ThemeDocument>) => void }) {
  ...
  const addSlider = () => {
    try {
      const pending = parseControls(JSON.parse(definitions ?? '[]'));
      let suffix = 1;
      while (pending.some(c => c.id === `detail-${suffix}`)) suffix++;
      setDefinitions(JSON.stringify(parseControls([...pending, { id: `detail-${suffix}`, label: 'Detail size', type: 'range', min: 0, max: 20, step: 1, value: 3 }]), null, 2));
      setError('');
  ...
  return <fieldset className="theme-role-settings"><legend>Theme-specific controls</legend>
    ...
    <details onToggle={e => { if (e.currentTarget.open) { setDefinitions(JSON.stringify(controls, null, 2)); setError(''); } }}>
      ...
      <button type="button" className="toolbar-button" onClick={addSlider}>Start with a slider</button>
