...
export const sortCommands = ["sort_az", "sort_za", "sort_az_case", "sort_za_case"] as const;
...
export function sortSelectedLines(state: EditorState, command: SortCommand): Transaction | null {
  ...
  function visit(node: ProseMirrorNode, pos: number): ProseMirrorNode {
    ...
    const children: { node: ProseMirrorNode; selected: boolean; group: string | null }[] = [];
    node.forEach((child, offset) => {
      const childPos = pos + 1 + offset;
      ...
      if (listNames.has(node.type.name)) group = "item";
        ...
        if (!header) group = "row";
      } else if (child.type.name === "paragraph" || child.type.name === "heading") group = child.type.name;
      // A table row is atomic; do not rearrange paragraphs within its cells.
      children.push({ node: node.type.name === "table" ? child : visit(child, childPos), selected, group });
    });
